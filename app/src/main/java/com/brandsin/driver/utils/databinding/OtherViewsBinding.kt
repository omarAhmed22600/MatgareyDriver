package com.brandsin.driver.utils.databinding
...
class OtherViewsBinding {
 ...
 @JvmStatic
        ...
        fun setImage(view: ImageView, url: String?) {
            when {
                url.isNullOrEmpty() -> {
                    Glide.with(view.context).load(R.drawable.logo4).error(R.drawable.logo4).into(view)
                }
                else -> {
                    Glide.with(view.context).load(url).error(R.drawable.logo4).into(view)
                }
            }
        }
        ...
        fun setStatus(view: TextView, status: String?) {
            when (status) {
                ...
                "accepted_by_driver" -> {
                    view.setTextColor(ContextCompat.getColor(view.context, R.color.order_prepared_color))
                    view.text = view.context.getString(R.string.accepted_by_driver)
                }
                "shipping" -> {
        ...
        fun registerLineState(view: ImageView, isEntered: Boolean?) {
            when (isEntered == false) {
                true -> {
                    Glide.with(view.context).load(R.drawable.ic_line_not_selected).error(R.drawable.logo4).into(view)
                }
                else -> {
                    Glide.with(view.context).load(R.drawable.ic_line_selected).error(R.drawable.logo4).into(view)
                }
            }
        }
