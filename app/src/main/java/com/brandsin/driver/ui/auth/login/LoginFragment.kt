...
import com.brandsin.driver.databinding.AuthFragmentLoginBinding
    ...
    override fun onChanged(it: Any?) {
        ...
        when (it) {
            ...
            Codes.REGISTER_INTENT -> {
                val action = LoginFragmentDirections.loginToRegister("0")
            ...
            Codes.SKIP_CLICKED -> {
                ...
                if (it is String) {
                    if (it == "not activate") {
                       ...
                       val action = LoginFragmentDirections.loginToVerify(viewModel.request.phone_number.toString(), viewModel.userID, "login")
                        findNavController().navigate(action)
                    } else {
